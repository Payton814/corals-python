...
if __name__ == "__main__":
    ...
    for iii in range(500):
        ...
        for i in SNR:
            ...
            c = CORALS(fn="coralsLPDA_impResponse.txt", th=th, SNR = i)
            #c = CORALS(fn="corals_halfscale_impulse_corr.txt", th=th)
            numSums = np.sum(np.abs(c.filter))
            #print(numSums)
            ...
            wf = c.getWaveform(noise_bool = True, SNR= i, bp_filt = True, Follow_Signal = Follow_Signal)    
    ...
    axs[2].plot(wf[0], mf)
    axs[0].axhline(th*5/wf[5], linestyle='--', color = 'r')
    axs[0].axhline(-th*5/wf[5], linestyle='--', color = 'r')
    plt.show()
